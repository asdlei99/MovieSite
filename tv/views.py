...
def tvDetail(request, *args, **kwargs):
    ...
    reply_obj = models.TvReply.objects.filter(tv__id=tid).order_by('-create_date')
    ...
    #判断是否已评过分
    if user_obj:
        has_rated = models.TvReply.objects.filter(tv__id=tid, user__id=user_obj.id, rating__gt=0)
    ...
    #收藏
    if user_obj:
        collect_obj = models.Collect_Tv.objects.filter(tv__id=tid, user__id=uid)
        if collect_obj:
            ret['collected'] = True
    ...
    #标签云
    tags_obj = models.Tv.objects.all().order_by('-week_visit_count').values('id','ch_name','director','actor')[:50]
    da_dict, mn_list = movie_helper.tagsCloud(tags_obj, 'tv')
    ret['da_dict'] = json.dumps(da_dict)
    ret['mn_list'] = json.dumps(mn_list)
    response = render_to_response('tv/tv_detail.html', ret, context_instance=RequestContext(request))
...
def tvIndex(request, *args, **kwargs):
    ...
    if target_type == 'at':
        ...
        for item in typedict:
            if target_type == item:
                type_result = models.Tv.objects.filter(types__contains = typedict[item])
    ...
    *标签云
    ...
    if thumb_switch == 't1':
        response = render_to_response('tv/tv_index.html', ret, context_instance=RequestContext(request))
    elif thumb_switch == 't2':
        response = render_to_response('tv/tv_index_t2.html', ret, context_instance=RequestContext(request))
    elif thumb_switch == 't3':
        response = render_to_response('tv/tv_index_t3.html', ret, context_instance=RequestContext(request))
    ...
    if request.method == 'POST':
        if uid:
            ...
            try:
                tv_obj = models.Tv.objects.get(id=tid)
                if not rating:
                    new_reply_obj = models.TvReply.objects.create(content=html_content, tv=tv_obj, user=user_obj)
                else:
                    reply_with_rating = models.TvReply.objects.filter(user__id=uid, tv__id=tid, rating__gt=0)
                    if not reply_with_rating.count():
                        new_reply_obj = models.TvReply.objects.create(rating=rating, content=html_content, tv=tv_obj, user=user_obj)
                    else:
                        ret['msg'] = '909'
                        return HttpResponse(json.dumps(ret))
            except Exception:
    ...
    if request.method == 'POST':
        if uid:
            ...
            try:
                ...
                new_rr_obj = models.TvReplyReply.objects.create(content=html_content, tv_reply=reply_obj, user=user_obj)
            except Exception:
...
def replyLike(request):
    ...
    if request.method == 'POST':
        if uid:
            ...
            if rid.isdigit():
                liked_obj_counts = models.TvReplyLike.objects.filter(user__id=uid, tv_reply_like__id=rid).count()
                if liked_obj_counts > 0: #该用户对该评论已赞
                    ...
                    try:
                        ...
                        return HttpResponse(json.dumps(ret))
                    reply_obj = models.TvReply.objects.get(id=rid)
                    models.TvReplyLike.objects.create(user=user_obj, tv_reply_like=reply_obj)
...
def getMoreReplies(request):
    if request.method == 'POST':
        ...
        if pref < 0:
            pref = 0
        rr_obj = models.TvReplyReply.objects.filter(tv_reply__id=rid).values('id','content','create_date','user_id')[pref:idx]
...
def getMoreComments(request):
    if request.method == 'POST':
        ...
        if xid.isdigit() and remainder.isdigit():
            ...
            if pref < 0:
                pref = 0
            reply_obj = models.TvReply.objects.filter(tv__id=xid).values('id','content','create_date','user_id')[pref:idx]
            res_list = [pref]
            for item in reply_obj:
                ...
                #获取该评论下的回复
                rr_obj = models.TvReplyReply.objects.filter(tv_reply__id=item['id']).values('id',
                ...
                for rr in rr_obj:  #获取每个回复的用户信息
                    ...
                    rr_list.append(rr)
                item['rr'] = rr_list
                item['rr_counts'] = models.TvReplyReply.objects.filter(tv_reply__id=item['id']).count()
                #评论的like
                item['like_counts'] = models.TvReplyLike.objects.filter(tv_reply_like__id=item['id']).count()
                res_list.append(item)
