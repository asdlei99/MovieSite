...
from packages import movie_helper, html_helper, user_helper
...
def animeDetail(request, *args, **kwargs):
    ...
    #右侧边栏
    billboard1 = models.Anime.objects.all().order_by('-week_visit_count')[0]
    billboard2 = models.Anime.objects.all().order_by('-week_visit_count')[1:10]
    ...
    ret['aid'] = aid
    try:
        anime_obj = models.Anime.objects.get(id=aid)
    except Exception:
        raise Http404
    
    anime_obj.visit_count += 1
    anime_obj.week_visit_count += 1
    anime_obj.month_visit_count += 1
    anime_obj.save()

    ret['detail'] = movie_helper.replaceWrongImg([anime_obj], 'ps')[0]
    
    down_list = []
    if anime_obj.down_urls and not anime_obj.down_urls == '无下载':
        name_list = anime_obj.down_names.splitlines()
        url_list = anime_obj.down_urls.splitlines()
    ...
    reply_obj = models.AnimeReply.objects.filter(anime__id=aid).order_by('-create_date')
    reply_counts = anime_obj.anime_r.all().count()
    ret['reply_counts'] = reply_counts
    ret['reply'] = reply_obj
    
    #判断是否已评过分
    if user_obj:
        has_rated = models.AnimeReply.objects.filter(anime__id=aid, user__id=user_obj.id, rating__gt=0)
    ...
    #收藏
    if user_obj:
        collect_obj = models.Collect_Anime.objects.filter(anime__id=aid, user__id=uid)
        if collect_obj:
            ret['collected'] = True
    ...
    '''
        猜你喜欢
    '''
    all_anime_obj = models.Anime.objects.exclude(id=aid)  #猜你喜欢中要排除自己
    anime_related_obj = movie_helper.relatedMovies(anime_obj, all_anime_obj)
    ret['related'] = movie_helper.replaceWrongImg(anime_related_obj, 'p')

    #标签云
    tags_obj = models.Anime.objects.all().order_by('-week_visit_count').values('id','ch_name','director','actor')[:50]
    da_dict, mn_list = movie_helper.tagsCloud(tags_obj, 'anime')
    ret['da_dict'] = json.dumps(da_dict)
    ret['mn_list'] = json.dumps(mn_list)
    response = render_to_response('anime/anime_detail.html', ret, context_instance=RequestContext(request))
...
def animeIndex(request, *args, **kwargs):
    ret = {'logined':False, 'show':'', 'pagelist':'', 'search_ok':False, 'counts':0}
    #判断用户是否登录，获取用户信息
    ret, user_obj = loginInfo(request, ret)
    ret['user_info'] = user_obj
    bill_obj = models.Anime.objects.all().order_by('-week_visit_count')
    ...
    #type
    if target_type == 'at':
        type_result = models.Anime.objects.all()
    else:
        typedict = {'juqing':'剧情', 'xiju':'喜剧', 'aiqing':'爱情', 'qihuan':'奇幻', 'guzhuang':'古装',
                'dongzuo':'动作', 'maoxian':'冒险', 'kehuan':'科幻', 'xuanyi':'悬疑', 'jingsong':'惊悚', 
                'kongbu':'恐怖', 'fanzui':'犯罪', 'zhanzheng':'战争', 
                #'donghua':'动画', 
                'jilupian':'纪录片',
                 'tongxing':'同性', 
                 #'qingse':'情色', 
                 'jiating':'家庭', 'ertong':'儿童', 'lishi':'历史', 'yundong':'运动', 
                 #'zhuanji':'传记'
        }
        for item in typedict:
            if target_type == item:
                type_result = models.Anime.objects.filter(types__contains = typedict[item])
    ...
    try:
        ...
        final_result = ''
    filter_url = '/anime/?focus=' + target_focus + '&type=' + target_type + '&region=' + target_region + '&p='
    ...
    *标签云
    ...
    if thumb_switch == 't1':
        response = render_to_response('anime/anime_index.html', ret, context_instance=RequestContext(request))
    elif thumb_switch == 't2':
        response = render_to_response('anime/anime_index_t2.html', ret, context_instance=RequestContext(request))
    elif thumb_switch == 't3':
        response = render_to_response('anime/anime_index_t3.html', ret, context_instance=RequestContext(request))
    ...
    if request.method == 'POST':
        if uid:
            ...
            try:
                anime_obj = models.Anime.objects.get(id=aid)
                if not rating:
                    new_reply_obj = models.AnimeReply.objects.create(content=html_content, anime=anime_obj, user=user_obj)
                else:
                    reply_with_rating = models.AnimeReply.objects.filter(user__id=uid, anime__id=aid, rating__gt=0)
                    if not reply_with_rating.count():
                        new_reply_obj = models.AnimeReply.objects.create(rating=rating, content=html_content, anime=anime_obj, user=user_obj)
                    else:
                        ret['msg'] = '909'
                        return HttpResponse(json.dumps(ret))
            except Exception,e:
...
def submitReplyReply(request):
    ...
    if request.method == 'POST':
        if uid:
            ...
            try:
                reply_obj = models.AnimeReply.objects.get(id=rid)
                new_rr_obj = models.AnimeReplyReply.objects.create(content=html_content, anime_reply=reply_obj, user=user_obj)
            except Exception:
...
def replyLike(request):
    ...
    if request.method == 'POST':
        if uid:
            ...
            if rid.isdigit():
                liked_obj_counts = models.AnimeReplyLike.objects.filter(user__id=uid, anime_reply_like__id=rid).count()
                if liked_obj_counts > 0: #该用户对该评论已赞
                    ...
                    try:
                        ...
                        return HttpResponse(json.dumps(ret))
                    reply_obj = models.AnimeReply.objects.get(id=rid)
                    models.AnimeReplyLike.objects.create(user=user_obj, anime_reply_like=reply_obj)
...
def getMoreReplies(request):
    if request.method == 'POST':
        ...
        if pref < 0:
            pref = 0
        rr_obj = models.AnimeReplyReply.objects.filter(anime_reply__id=rid).values('id','content','create_date','user_id')[pref:idx]
...
def getMoreComments(request):
    if request.method == 'POST':
        ...
        if xid.isdigit() and remainder.isdigit():
            ...
            if pref < 0:
                pref = 0
            reply_obj = models.AnimeReply.objects.filter(anime__id=xid).values('id','content','create_date','user_id')[pref:idx]
            res_list = [pref]
            for item in reply_obj:
                ...
                #获取该评论下的回复
                rr_obj = models.AnimeReplyReply.objects.filter(anime_reply__id=item['id']).values('id',
                ...
                for rr in rr_obj:  #获取每个回复的用户信息
                    ...
                    rr_list.append(rr)
                item['rr'] = rr_list
                item['rr_counts'] = models.AnimeReplyReply.objects.filter(anime_reply__id=item['id']).count()
                #评论的like
                item['like_counts'] = models.AnimeReplyLike.objects.filter(anime_reply_like__id=item['id']).count()
                res_list.append(item)
