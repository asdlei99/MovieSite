 ...
 * 
 */
window.cate = /^http:\/\/.*?\/([a-z]+)\/.*$/.exec(window.location.href)[1]
$(function() {
	//点击panel变色
	var $panelTitle = $('#accordion1 .panel-title a');
	var $panel = $('#accordion1 > .panel');
	$panelTitle.bind('click',function(event) {
		$thisPanel = $(this).parent().parent().parent();
		if($thisPanel.hasClass('panel-m1')) {
			$thisPanel.removeClass('panel-m1');
		} else {
			$panel.each(function() {
				$(this).removeClass('panel-m1');
			})
			$thisPanel.addClass('panel-m1');
		}
