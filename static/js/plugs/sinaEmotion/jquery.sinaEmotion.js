...
$(function() {
	$.ajax( {
		dataType : 'json',
		url : '/static/js/plugs/sinaEmotion/json/jquery.sinaEmotion.json',
	...
	if(typeof (s) != "undefined") {
		...
		for(var i = 0; i < sArr.length; i++){
			if(uSinaEmotionsHt.containsKey(sArr[i])) {
				var iconFileName = uSinaEmotionsHt.get(sArr[i])
				var reStr = '<img src="/static/js/plugs/sinaEmotion' + iconFileName.replace(/^\./,'',iconFileName) + '" height="22" width="22">';
...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			$('#emotions').html('<div style="float:right"><a href="javascript:void(0);" id="prev">&laquo;</a><a href="javascript:void(0);" id="next">&raquo;</a></div><div class="categorys">正在加载，请稍后...</div><div class="container"></div><div class="page"></div>');
		...
		function showEmotions(){
			...
			for(var i = page * 72; i < (page + 1) * 72 && i < emotions[category].length; ++i){
				$('#emotions .container').append($('<a href="javascript:void(0);" title="' + emotions[category][i].name + '"><img src="/static/js/plugs/sinaEmotion/' + emotions[category][i].icon + '" alt="' + emotions[category][i].name + '" width="22" height="22" /></a>'));
			}
